...
| **P2** Task construction + metric modules | biostat | **claimed by agent-sonnet5 on biostat @ 2026-09-08T20:22Z** | agent-sonnet5 | `feat/p02-tasks-metrics` | Stubs to fill: `data/tasks.py`, `shards.py`, `collate.py`, `qa_synth.py`; `eval/{detection,qa,summary,efficiency,aggregate}.py`. **Done already:** `eval/{grounding,phase,rsd,reliability}.py` + `data/{templates,regime}.py` + tests. Stand-in: synthetic surgical timelines in `data/standin.py` (parses Charades-STA txt if present, else deterministic synthetic) — P1 parsers untouched. |
